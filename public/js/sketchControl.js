...
function setup(){
  ...
  bShowOn = createButton('Show On/Off').size(60,bHeight).position(width - 100, yPos).mousePressed(showOn);
  if(!showStarted){
  ...
  yPos+=bHeight+5;
  bSing = createButton('Sing For Me!').position(marginX, yPos).size(200,bHeight).mousePressed(singForMe);
...
function turnGroup(group,val){
  for(let i=0;i<bots.length;i++){
    // console.log(botPersonality[i]);
    if(botPersonality[i]==group){
      var message=[bots[i],val];
      socket.emit('target',message);
      console.log("Turning "+bots[i]+" "+val);
    }
  }
}
...
function turnGroupRan(group,val){
  ...
  for(let i=0;i<bots.length;i++){
    if(botPersonality[ranNumbers[i]]==group){
      var message=[bots[ranNumbers[i]],val];
      socket.emit('target',message);
      return;
    }
  }
